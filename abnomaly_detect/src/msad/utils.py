...
def get_eval_loaders(dataset, label_class, batch_size, backbone):
    if dataset == "cifar10":
        ds = torchvision.datasets.CIFAR10
        transform = transform_color if backbone == 152 else transform_resnet18
        coarse = {}
        ...
        testset.data = attacked_data_np.astype(np.uint8)

        idx = np.array(trainset.targets) == label_class

        test_idx = np.array(testset.targets) == label_class
        testset.data = testset.data[test_idx]
        testset.targets = [testset.targets[i] for i, flag in enumerate(test_idx, 0) if flag]
