...
from datasets.datasets_test import get_normal_dataset_test, get_test_loader_one_vs_all, get_test_loader_one_vs_one, get_all_label_normal_dataset_test, get_test_loader_all_vs_all
...
def eval_all_vs_all(args, model, device, train_feature_space):
    test_loader_one_vs_all = get_test_loader_all_vs_all(args.dataset, args.normal_data_path,
                                                        args.download_dataset, args.eval_batch_size, args.attacked_data_file)
    test_feature_space, test_labels = extract_feature_space(model, device, test_loader_one_vs_all)
    distances = knn_score(train_feature_space, test_feature_space)
    auc = roc_auc_score(test_labels, distances)
    print(f'AUROC on the All-vs-All setting is: {auc}')
    return auc
...
def evaluate_model(args, model, device):
    ...
    model.eval()
    if args.all_label:
        normal_train_loader = get_all_label_normal_dataset_test(args.dataset, args.normal_data_path,
                                                    args.download_dataset, args.eval_batch_size)
        ...
        auc_all_vs_all = eval_all_vs_all(args, model, device, train_feature_space)
        final_results['all_vs_all'] = auc_all_vs_all
